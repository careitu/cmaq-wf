...
def get_lat_lon(dom, month):
    ...
    DIR_MCIP = _join(proj1.path.mcip, '{}km'.format(dom.size),
                     dom.name, month_name + '_monthly')
    CRO_FILE = fmt_cro.format(proj1.name, dom.size, dom.name,
                              year, month)
    ...
    for dom_name, d in doms.items():
        for i, a in enumerate(d.transpose('pol_name', ...)):
            ...
            if cb_limits is None:
                vmin = x.quantile(0.02).data.tolist()
                vmax = x.quantile(0.98).data.tolist()
                ext = (vmax - vmin) * 0.20
                vmax = vmax + ext
                vmin = vmin - ext
                if vmax <= 0:
            ...
            norm = mcolors.DivergingNorm(vmin=vmin, vcenter=0,
                                         vmax=vmax)
            ...
            for i, ax in enumerate(p.axes.flat):
                if stats is None:
                    ax.set_title(calendar.month_name[month[i]].title())
...
def calc_stat(dom_names, pol_names, year, months, proj, stats=None):
    """
...
pth = _join('plots_combined_diff', '_'.join(('plots', cmap_str, no_limit_str,
                                             proj_name2, proj_name1)))

if NO_LIMIT:
    CB_LIMITS = None
else:
    CB_LIMITS = {'CO': [-100, 100],
                 'NOX': [-100, 100],
                 'O3': [-100, 100],
                 'PM10': [-75, 30],
                 'PM25_TOT': [-100, 100],
...
Domain = namedtuple('domain', ['key', 'proj1', 'proj2'])

stats = ['mean']
p1_doms = calc_stat(DOM_NAMES, POL_NAMES, year, months, proj1, stats)
p2_doms = calc_stat(DOM_NAMES, POL_NAMES, year, months, proj2, stats)
dif_doms = {el: None for el in DOM_NAMES}
for i in [Domain(k, v, p2_doms[k]) for k, v in p1_doms.items()]:
    dif_doms[i.key] = 100 * (i.proj2 - i.proj1) / i.proj1
    print(i.key)
    print(dif_doms[i.key].max())
    print(dif_doms[i.key].min())

plot_map(dif_doms, pth, cmap, CB_LEVELS, cb_limits=CB_LIMITS)
