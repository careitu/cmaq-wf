...
def get_lat_lon(proj, dom, month):
    """
...
def calc_stat(proj_names, dom_names, pol_names, year, months, stats=None):
    """
    ...
    def get_monthly_stats_for_file(proj, dom_name, pol_names, months):
        ...
        for m in months:
            nco = Dataset(_join(proj.path.post,
                          fmt_com.format(
                            ...
                            year, m)))
            lon, lat = get_lat_lon(proj, dom, m)
    ...
    for dn in doms.keys():
        projs = {i: None for i in proj_names}
        for pn in proj_names:
            proj = s.get_proj_by_name(pn)
            pols = get_monthly_stats_for_file(proj, dn, pol_names, months)
            projs[pn] = xr.concat(list(pols.values()),
                                  pd.Index(list(pols.keys()), name="pol_name"))
            projs[pn].attrs['units'] = 'ugm-3'
        doms[dn] = xr.concat(list(projs.values()),
                             pd.Index(list(projs.keys()), name="proj"))
        doms[dn] = doms[dn].assign_coords(domain=dn)
    return doms
...
pth = _join('plots_by_proj', cmap_str, no_limit_str)
...
doms = calc_stat(PROJ_NAMES, DOM_NAMES, POL_NAMES, year, months)
