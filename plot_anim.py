...
def get_data(dom_names, pol_names, months, tstep=True):
...
CB_LIMITS = None if NO_LIMIT else {'CO': [0, 300],
                                   'NOX': [0, 70],
                                   'O3': [0, 80],
                                   'PM10': [0, 100],
                                   'PM25_TOT': [0, 250],
...
for dn in DOM_NAMES:
    for pn in POL_NAMES:
        file_name = '_'.join((proj.name, dn, pn)) + '.mp4'
        if not _isfile(file_name):
            print(file_name)
            cbar_label = ' '.join((POL_LABELS[pn], POL_UNITS[pn]))
            dat = get_data(dn, pn, months, tstep=True)
            x = next(dat)
